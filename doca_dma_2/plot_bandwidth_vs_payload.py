    ...
    if file.startswith(file_name + '_') and file.endswith('.json'):
        try:
            ...
            for group_idx, group in enumerate(data):
                ...
                bandwidths = [entry['bandwidth'] * 8 / 1024 / 1024 / 1024 for entry in group]
                ops = group[0]['ops'] 
                
                plt.plot(file_sizes, bandwidths, color=colors[group_idx % len(colors)], label=f'operations={ops}', marker='o', alpha=1, markersize=5, linewidth=2.5)
            ...
            save_path = save_dir + '/' + file_name + '_' + 'bandwidth' + '_' + str(num) + '.png'
