...
uint64_t F_HASH(level_hash *level, const uint8_t *key) {
  return (hash_((void *)key, strlen(key), level->f_seed));
}
...
uint64_t S_HASH(level_hash *level, const uint8_t *key) {
  return (hash_((void *)key, strlen(key), level->s_seed));
}
...
level_hash *level_init(uint64_t level_size) {
  ...
  level->buckets[1] =
      alignedmalloc(pow(2, level_size - 1) * sizeof(level_bucket));
...
void level_expand(level_hash *level) {
  ...
  level_bucket *newBuckets =
      alignedmalloc(level->addr_capacity * sizeof(level_bucket));
  if (!newBuckets) {
...
void level_shrink(level_hash *level) {
  ...
  level->level_size--;
  level_bucket *newBuckets =
      alignedmalloc(pow(2, level->level_size - 1) * sizeof(level_bucket));
  ...
  for (old_idx = 0; old_idx < pow(2, level->level_size + 1); old_idx++) {
    for (i = 0; i < ASSOC_NUM; i++) {
      if (interimBuckets[old_idx].token[i] == 1) {
        if (level_insert(level, interimBuckets[old_idx].slot[i].key,
                         interimBuckets[old_idx].slot[i].value)) {
...
uint8_t level_insert(level_hash *level, uint8_t *key, uint8_t *value) {
  ...
  return level_insert(level, key, value);
}
