...
{
  uint8_t
      token[ASSOC_NUM]; // A token indicates whether its corresponding slot is
                        // empty, which can also be implemented using 1 bit
  entry slot[ASSOC_NUM];
} level_bucket;

typedef struct level_hash { // A Level hash table
  level_bucket
      *buckets[2]; // The top level and bottom level in the Level hash table
  uint64_t level_item_num[2]; // The numbers of items stored in the top and
  ...
  uint8_t level_expand_time; // Indicate whether the Level hash table was
                             // expanded, ">1 or =1": Yes, "0": No;
  uint8_t resize_state; // Indicate the resizing state of the level hash table,
                        // ‘0’ means the hash table is not during resizing; ‘1’
                        // means the hash table is being expanded; ‘2’ means the
                        // hash table is being shrunk.
  uint64_t f_seed;
